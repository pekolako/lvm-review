...
eta.simulate <- function(n_sites, n_species, predictors=FALSE, n_lv=2) {
...
y.simulate <- function(eta, link="probit") {
  n = dim(eta)[1]
  p = dim(eta)[2]
  y = matrix(0,n,p)
  z <- 1 
...
gllvm.sim <- function(true.vals, j, n.sim=100, link="probit") {
  ...
  while(i <= (n.sim-1+j)) {
    ...
    yi = y.simulate(eta.true, link)
    proc.lami = proc.Ui = t.fiti = rep(NA, 7)
    betai = matrix(NA, nrow=p, ncol=7)
    VAi = LAi = EVAi = MCMCi = PQLi = EMi = NULL
    # fit the models:
    # gllvm-VA
    t.fiti[1] = system.time(try({
      VAi = gllvm(yi, family="binomial", sd.errors = FALSE)
    ...
    t.fiti[2] = system.time(try({
      LAi = gllvm(yi, family="binomial", sd.errors = FALSE, method="LA", link = link)
    ...
    t.fiti[3] = system.time(try({
      EVAi = gllvm(yi, family="binomial", sd.errors = FALSE, method="EVA", link = link)
    ...
    t.fiti[4] = system.time(try({
      MCMCi = boral(yi, lv.control = list(num.lv=2), family="binomial")
      lami = MCMCi$lv.coefs.mean[,2:3]
      Ui = MCMCi$lv.mean
    ...
    t.fiti[5] = system.time(try({
      PQLi = gmf(yi, family=binomial(link), intercept=TRUE, verbose=FALSE)
      lami = PQLi$v
      Ui = PQLi$u
    ...
    t.fiti[6] = system.time(try({
      EMi = ltm(yi ~ z1 + z2, constraint = matrix(c(1,3,0),byrow=TRUE,ncol=3))
      lami = EMi$coefficients[,2:3]
      Ui = factor.scores(EMi)$score.dat[,c("z1","z2")]
      ...
      betai[,6] = EMi$coefficients[,1]
      },silent = TRUE))[3]
    # glmmTMB
    t.fiti[7] = system.time(try({
      y0 = y00 = data.frame(yi)
      y0$id = 1:n
      # reshape to long format in order to use glmmTMB
      ...
      RRi = glmmTMB(abund ~ Species + 0 + rr(Species + 0|id, d=num.lv),
                                       data = y0l, family=binomial(link="probit"))
      Ui = ranef(RRi)$cond$id[,1:num.lv]
      proc.Ui[7] = vegan::procrustes(scale(U.true, scale=TRUE),scale(Ui, scale=TRUE))$ss
      ...
      rownames(lami) = ord1
      lami = lami[ord0,]
      proc.lami[7] = vegan::procrustes(scale(lam.true, scale=TRUE),scale(lami, scale=TRUE))$ss
    },silent=FALSE))[3]
  ...
  colnames(proc.U) = colnames(proc.lam) = colnames(t.fit) = 
    colnames(beta) = c("VA", "LA", "EVA", "MCMC", "PQL", "EM", "LA2")
...
# a) n=50 obs/rows, m=50 species/cols, 2 LVs.
ptime1 <- system.time({
  tvals50x50 <- eta.simulate(n_sites = 50, n_species = 50, predictors = FALSE, n_lv = 2)
  rev.50.50 <- foreach(j = ss,.packages=c("vegan","MASS")) %dopar% {
    gllvm.sim(tvals50x50, j, n.sim=5)
  };
})[3]
save(rev.50.50,file = "./lvmrev_50x50.Rdata") # save the results
save(tvals50x50,file="./tvals50x50.Rdata")    # save true values for bias/rmse

# b) n=50 obs/rows, m=150 species/cols, 2 LVs.
ptime2 <- system.time({
  tvals50x150 <- eta.simulate(n_sites = 50, n_species = 150, predictors = FALSE, n_lv = 2)
  rev.50.150 <- foreach(j = ss,.packages=c("vegan","MASS")) %dopar% {
    gllvm.sim(tvals50x150, j, n.sim=5)
  };
})[3]
save(rev.50.150,file = "./lvmrev_50x150.Rdata")
save(tvals50x150,file="./tvals50x150.Rdata")

# c) n=200 obs/rows, m=50 species/cols, 2 LVs.
ptime3 <- system.time({
  tvals200x50 <- eta.simulate(n_sites = 200, n_species = 50, predictors = FALSE, n_lv = 2)
  rev.200.50 <- foreach(j = ss,.packages=c("vegan","MASS")) %dopar% {
    gllvm.sim(tvals200x50, j, n.sim=5)
  };
})[3]
save(rev.200.50,file = "./lvmrev_200x50.Rdata")
save(tvals200x50,file="./tvals200x50.Rdata")


# d) n=200 obs/rows, m=150 species/cols, 2 LVs.
ptime4 <- system.time({
  tvals200x150 <- eta.simulate(n_sites = 200, n_species = 150, predictors = FALSE, n_lv = 2)
  rev.200.150 <- foreach(j = ss,.packages=c("vegan","MASS")) %dopar% {
    gllvm.sim(tvals200x150, j, n.sim=5)
  };
})[3]
save(rev.200.150,file = "./lvmrev_200x150.Rdata")
save(tvals200x150,file="./tvals200x150.Rdata")